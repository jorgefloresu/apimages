...
define('NO_XML', true);
define('DEF_NUMBER_OF_ROWS', 10);
define('DEF_MEDIA_SIZE', 450);


class Getprovidersall extends CI_Controller {
        ...
        function search($page=0)
        {
        	...
        	if ( !empty($data['keywords']) )
        	{
                ...
                $providers['Fotosearch'] = 0;
                $providers['Depositphoto'] = 0;
                $providers['Fotolia'] = 0;
                $providers['Ingimages'] = 0;

                foreach ($providers as $key => $provider) {
                    $rows[$key] = $this->SearchOn($key, $data, $page);
                }
                $faltan = 0;
                foreach ($rows as $key => $value) {
                    if ($value <= DEF_NUMBER_OF_ROWS) {
                        $faltan += DEF_NUMBER_OF_ROWS - $value;
                        unset($providers[$key]);
                    }
                }
        ...
        function SearchOn($provider, &$data, $page, $addremain=0)
        {
            switch ($provider) {
