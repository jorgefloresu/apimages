...
class Dreamstime_Api
{
    ...
    public function getMediaData($mediaId)
    {
        ...
        $url = $this->getFullURI(DreamstimeParams::MEDIA_DATA_CMD, $postParams);
        return $this->checkResponse($this->post($url, $postParams));
    }
    ...
    */
    public function getMediaPreview($mediaId, $mediaSize=DEF_MEDIA_SIZE)
    {
        ...
        $url = $this->getFullURI(DreamstimeParams::ASSET_PREVIEW_CMD, $postParams);
        return $url;              
    }
    ...
    {
         $postParams = array(
            DreamstimeParams::SALES_CURRENCY => 'USD',
            DreamstimeParams::SALES_TYPE => $plan,
            DreamstimeParams::MEDIA_IMAGE_CODE => $media,
            DreamstimeParams::DOWNLOAD_FORMAT => $type,
            DreamstimeParams::SALES_ORDER_ID => date("Ymd").str_pad($orderId, 4, "0", STR_PAD_LEFT),
            DreamstimeParams::SALES_USER_ID => $userId,
            DreamstimeParams::SALES_PRICE => $price,
            DreamstimeParams::API_KEY => $this->apiKey,
            DreamstimeParams::API_PASSWORD => $this->apiPwd
           );

        $url = $this->getFullURI(DreamstimeParams::SALES_REPORT_CMD, $postParams);
        return $this->checkResponse($this->post($url, $postParams));
    }
    ...
    {
         $postParams = array(
            DreamstimeParams::DOWNLOAD_GET_TOKEN => '1',
            DreamstimeParams::MEDIA_IMAGE_CODE => $media,
            DreamstimeParams::DOWNLOAD_FORMAT => $type,
            DreamstimeParams::DOWNLOAD_REF => $downloadRef,
            DreamstimeParams::API_KEY => $this->apiKey,
            DreamstimeParams::API_PASSWORD => $this->apiPwd
           );

        $url = $this->getFullURI(DreamstimeParams::DOWNLOAD_ASSET, $postParams);
        //return $url;
    ...
    public function goDownload($media, $downloadRef, $token)
    {
        $postParams = array(
            DreamstimeParams::MEDIA_IMAGE_CODE => $media,
            DreamstimeParams::DOWNLOAD_REF => $downloadRef,
            DreamstimeParams::DOWNLOAD_TOKEN => $token
            );
