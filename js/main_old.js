  ...
  $.each(init.providers, function(i, provider){
      ...
      var jqXHR = $.ajax({
              ...
              timeout: init.timeoutLimit
                ...
                console.log(provider+' done!: '+msg.result[provider].data.length+' resultados');
                if (msg.result[provider].data.length>0){                  
          ...
          jqXHR.done(function(msg, textStatus){
                ...
                console.log("finish "+provider+", result: "+res_count);
            ...
            console.log("fail "+provider+": "+textStatus+'. error:'+errorThrown);
            if (textStatus == 'timeout')
  ...
  function downloadUrlRef(provider, size, resolution_id, image_code, license, price, urlPrev) {
    ...
    switch (provider){
      case "Fotosearch":
        ...
        break;
      case "Depositphoto":
        collection_str = size+"/"+license+"/"+username;
        break;
      case "Ingimages":
